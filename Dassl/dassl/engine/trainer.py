...
class TrainerBase:
    ...
    def train(self, start_epoch, max_epoch, idx=-1,global_epoch=-1,is_fed=False):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            self.before_epoch()
            self.run_epoch(idx,global_epoch)
            self.after_epoch()
...
class SimpleTrainer(TrainerBase):
    ...
    def train(self,idx=-1,global_epoch=0,is_fed=False):
        super().train(self.start_epoch, self.max_epoch,idx,global_epoch,is_fed)
    ...
    @torch.no_grad()
    def test(self, split=None, is_global=False, current_epoch=0, idx=-1):
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self,idx=-1,global_epoch=-1):
        self.set_model_mode("train")
        losses = MetricMeter()
        batch_time = AverageMeter()
        data_time = AverageMeter()
